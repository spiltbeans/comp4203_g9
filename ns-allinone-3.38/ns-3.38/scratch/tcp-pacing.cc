...
static void
NodeZeroOutput(Ptr<const Packet> p, Ptr<Ipv4> ipv4, uint32_t interface)
{

    packetTraceStream << std::fixed << std::setprecision(6) << Simulator::Now().GetSeconds()
                      << " Node-0-Output " << p->GetSize() << " uid " << p->GetUid() << std::endl;
}
static void
NodeZeroInput(Ptr<const Packet> p, Ptr<Ipv4> ipv4, uint32_t interface)
{

    packetTraceStream << std::fixed << std::setprecision(6) << Simulator::Now().GetSeconds()
                      << " Node-0-Input " << p->GetSize() << " uid " << p->GetUid() << std::endl;
}
static void
NodeThreeInput(Ptr<const Packet> p, Ptr<Ipv4> ipv4, uint32_t interface)
{
    packetTraceStream << std::fixed << std::setprecision(6) << Simulator::Now().GetSeconds()
                      << " Node-3-Input " << p->GetSize() << " uid "<< p->GetUid() << std::endl;
}

static void
NodeThreeOutput(Ptr<const Packet> p, Ptr<Ipv4> ipv4, uint32_t interface)
{
    packetTraceStream << std::fixed << std::setprecision(6) << Simulator::Now().GetSeconds()
                      << " Node-3-Output " << p->GetSize() << " uid "<< p->GetUid() << std::endl;
}
/*static void
NodeZeroDropped(const Ipv4Header &ipp, Ptr<const Packet> p, DropReason drop, Ptr<Ipv4> ipv4, uint32_t interface){
   packetTraceStream << std::fixed << std::setprecision(6) << "Dropped "<< p->GetUid() << drop() << std::endl;
}
*/
void
ConnectSocketTraces()
...
//
    ...
    ApplicationContainer sinkApps4 = packetSinkHelper.Install(c.Get(3)); // n4 as sink
